...
void pokey_write(pokey_t *p, int reg, uint8_t data)
{
    switch (reg) {
        ...
        case 9: /* STIMER: restart the dividers */
            ...
            break;
        case 11: /* POTGO: dump the capacitors and start all eight pots counting again */
            p->pot_count = 0;
            p->pot_done = 0;
            for (int i = 0; i < 8; i++) if (p->pot_value[i] == 0) p->pot_done |= (uint8_t)(1 << i);
            break;
...
{
    switch (reg & 0x0f) {
        case 0x0a: {                                   /* RANDOM: 8 bits of the 17-bit (or 9-bit) polynomial */
            gen_polys();
            uint8_t v = 0;
            for (int i = 0; i < 8; i++) v = (uint8_t)((v << 1) | ((p->audctl & 0x80) ? poly9[(clock + i) % POLY9_LEN] : poly17_at(clock + i)));
            return (uint8_t)~v;
        }
        case 0x08: return (uint8_t)~p->pot_done;       /* ALLPOT: a set bit is still counting */
        case 0x0e: return 0xff;                        /* IRQST */
        case 0x0f: return 0xff;                        /* SKSTAT */
        default:                                       /* POT0-7 */
            if ((reg & 0x0f) < 8) {
                uint8_t v = p->pot_value[reg & 0x0f];
                return (p->pot_count >= v) ? v : (uint8_t)p->pot_count;
            }
            return 0x00;
    }
}
...
{
    if (p->pot_count < 255) p->pot_count++;
    for (int i = 0; i < 8; i++)
        if (p->pot_count >= p->pot_value[i]) p->pot_done |= (uint8_t)(1 << i);
}
